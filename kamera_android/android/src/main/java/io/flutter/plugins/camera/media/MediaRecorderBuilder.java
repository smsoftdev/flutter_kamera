...
public class MediaRecorderBuilder {
  ...
  private final CamcorderProfileX camcorderProfileX;
  ...
  public MediaRecorderBuilder(
      @NonNull CamcorderProfileX camcorderProfileX, @NonNull String outputFilePath) {
    this(camcorderProfileX, outputFilePath, new MediaRecorderFactory());
  }
  ...
  MediaRecorderBuilder(
    ...
    this.camcorderProfileX = null;
    this.encoderProfiles = null;
    this.recorderFactory = helper;
  }

  MediaRecorderBuilder(
      @NonNull CamcorderProfileX camcorderProfileX,
      @NonNull String outputFilePath,
      MediaRecorderFactory helper) {
    this.outputFilePath = outputFilePath;
    this.camcorderProfile = null;
    this.camcorderProfileX = camcorderProfileX;
    this.encoderProfiles = null;
    this.recorderFactory = helper;
  }
  ...
  public MediaRecorder build() throws IOException, NullPointerException, IndexOutOfBoundsException {
    ...
    if (camcorderProfileX != null) {
      mediaRecorder.setOutputFormat(camcorderProfileX.fileFormat);
      if (enableAudio) {
        mediaRecorder.setAudioEncoder(camcorderProfileX.audioCodec);
        mediaRecorder.setAudioEncodingBitRate(camcorderProfileX.audioBitRate);
        mediaRecorder.setAudioSamplingRate(camcorderProfileX.audioSampleRate);
      }
      mediaRecorder.setVideoEncoder(camcorderProfileX.videoCodec);
      mediaRecorder.setVideoEncodingBitRate(camcorderProfileX.videoBitRate);
      mediaRecorder.setVideoFrameRate(camcorderProfileX.videoFrameRate);
      mediaRecorder.setVideoSize(
          camcorderProfileX.videoFrameWidth, camcorderProfileX.videoFrameHeight);
    }
