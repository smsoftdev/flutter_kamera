...
import io.flutter.plugins.camera.types.CamcorderProfileX;
...
public class ResolutionFeature extends CameraFeature<ResolutionPreset> {
  ...
  private CamcorderProfileX recordingProfileLegacyX;
  ...
  public CamcorderProfileX getRecordingProfileLegacyX() {
    return this.recordingProfileLegacyX;
  }
  ...
  static Size computeBestPreviewSize(int cameraId, ResolutionPreset preset)
    ...
    if (100 <= cameraId) {
      CamcorderProfileX profile = createCamcorderProfileX();
  ...
  public static CamcorderProfile getBestAvailableCamcorderProfileForResolutionPresetLegacy(
    ...
    switch (preset) {
      ...
      case veryHigh:
        if (CamcorderProfile.hasProfile(cameraId, CamcorderProfile.QUALITY_1080P)) { // 1920 x 1088
          return CamcorderProfile.get(cameraId, CamcorderProfile.QUALITY_1080P);
        }
      case high:
        if (CamcorderProfile.hasProfile(cameraId, CamcorderProfile.QUALITY_720P)) { // 1280 x 720
      ...
      case low:
        if (CamcorderProfile.hasProfile(cameraId, CamcorderProfile.QUALITY_QVGA)) { // 320 x 240
      ...
      default:
        ...
        throw new IllegalArgumentException(
            "No capture session available for current capture session for resolution " + preset + " cameraId "
                + cameraId + ".");
    }
  }
  ...
  private static CamcorderProfileX createCamcorderProfileX() {
    CamcorderProfileX camcorderProfileX = new CamcorderProfileX(
      2, // duration
      CamcorderProfile.QUALITY_720P, // quality
      0, // fileFormat
      0, // videoCodec
      2000000, // videoBitRate
      30, // videoFrameRate
      1280, // videoFrameWidth
      720, // videoFrameHeight
      0, // audioCodec
      20000, // audioBitRate
      19200, // audioSampleRate
      2 // audioChannels
      );
    return camcorderProfileX;
  }
  ...
  private void configureResolution(ResolutionPreset resolutionPreset, int cameraId)
    ...
    if (100 <= cameraId) {
      @SuppressWarnings("deprecation")
      CamcorderProfileX camcorderProfileX = createCamcorderProfileX();
      recordingProfileLegacyX = camcorderProfileX;
      captureSize = new Size(recordingProfileLegacyX.videoFrameWidth, recordingProfileLegacyX.videoFrameHeight);
    } else if (Build.VERSION.SDK_INT >= 31) {
